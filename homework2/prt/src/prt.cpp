...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            std::cout << "Rander Type Interreflection." << '\n';
            std::function<std::unique_ptr<std::vector<double>>(const Point3f& v, const Normal3f& n, int bounce)> func = [&](const Point3f& v, const Normal3f& n, int bounce) -> std::unique_ptr<std::vector<double>> {
                ...
                for (int t = 0; t < sample_size; t++)
                {
                    ...
                    {
                        double alpha = (t + rng(gen)) / sample_size;
                        double beta = (p + rng(gen)) / sample_size;
                        double phi = 2.0 * M_PI * beta;
                        double theta = acos(2.0 * alpha - 1.0);
                        Eigen::Array3d d = sh::ToVector(phi, theta); 
                        Vector3f wi = Vector3f(d.x(), d.y(), d.z());
                        double H = wi.normalized().dot(n.normalized());
                        nori::Intersection its;
                        if (H > 0.0 && scene->rayIntersect(Ray3f(v, wi.normalized()), its))
                        {
                            nori::MatrixXf normal = its.mesh->getVertexNormals();
                            Point3f index = its.tri_index;
                            Point3f hitPos = its.p;
                            Vector3f bary = its.bary;

                            Normal3f hitNormal = Normal3f(normal.col(index.x()).normalized() * bary.x() + normal.col(index.y()).normalized() * bary.y() + normal.col(index.x()).normalized() * bary.z());
                            hitNormal.normalized();

                            auto nextCoeffs = func(p, hitNormal, bounce + 1);

                            for (int i = 0; i < SHCoeffLength; i++)
                            {
                                auto interpolateSH = (m_TransportSHCoeffs.col(index.x()).coeffRef(i) * bary.x() +
                                                        m_TransportSHCoeffs.col(index.y()).coeffRef(i) * bary.y() +
                                                        m_TransportSHCoeffs.col(index.z()).coeffRef(i) * bary.z());
                                (*result)[i] += (interpolateSH + (*nextCoeffs)[i]) * H;
                            }
                        }
                    }
                }
                for (int i = 0; i < result->size(); i++)
                    (*result)[i] /= sample_size * sample_size;
                return result;
            };
            int last = 0, now = -1;
            std::cout << last << "\n";
            for (int i = 0; i < mesh->getVertexCount(); i++)
            {
                ...
                now = float(i) / (float)mesh->getVertexCount() * 100;
                const Point3f &v = mesh->getVertexPositions().col(i);
                const Normal3f &n = mesh->getVertexNormals().col(i);
                auto getCoeffs = func(v, n, 0);
                for (int j = 0; j < SHCoeffLength; j++)
                {
                    m_TransportSHCoeffs.col(i).coeffRef(j) += (*getCoeffs)[j];
                }
            }
            std::cout << "Interreflection finished." << '\n';
        }
